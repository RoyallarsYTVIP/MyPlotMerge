...
class EventListener implements Listener
{
	...
	private function onEventOnBlock($event) : void {
		...
		$merge = new Config($this->plugin->getDataFolder() . "merge_" . $block->getLevel()->getName() . ".yml", 2);
		if($plot !== null) {
		...
		} elseif($event->getPlayer()->hasPermission("myplot.admin.build.road") or $this->plugin->isPlotMerged($block, $player)) { //ismergePlayer $player,
			return;
        }
		//elseif ($merge->exists("$plot")) {
			//$event->getPlayer()->sendMessage("IsMerged: true");
		//return;
		//else {
		//	$event->getPlayer()->sendMessage("IsMerged: false");
		 elseif($this->plugin->isPositionBorderingPlot($event->getBlock()) and $this->plugin->editMergeRand($event->getPlayer())) {
	...
	public function onPlayerMove(PlayerMoveEvent $event) : void {
		...
		if($plot !== null and $plot !== $this->plugin->getPlotByPosition($event->getFrom())) {
			...
			if($plot->owner !== "") {
				$owner = $plot->owner;
				$popup = TextFormat::GRAY . "Besitzer: " . TextFormat::GOLD . $owner . TextFormat::GRAY . ". ID: " . TextFormat::YELLOW . $plot;
			}else{
				$popup = TextFormat::GRAY . "Kein Besitzer. ID: " . TextFormat::YELLOW . $plot . "\n" . TextFormat::YELLOW . "/p claim um es zu erhalten.";
			}
