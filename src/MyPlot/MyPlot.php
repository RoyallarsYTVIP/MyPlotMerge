...
use MyPlot\events\MyPlotTeleportEvent; //isSame
...
class MyPlot extends PluginBase
{
	...
	public function isTrusted(Player $player, $plot){
		...
		return ($plot->owner == $username or $plot->isHelper($username) or $plot->isHelper("*") or $player->hasPermission("myplot.admin.build.plot"));
	}
	...
	//MergeAPI Anfang
	public function mergePlot(Plot $plot , Plot $plot2 , Player $player, $direction, int $maxBlocksPerTick = 256) : bool
	{
		$this->newRandPlot($plot, 256, 0, 1);
		$this->newRandPlot($plot2, 256, 0, 1);
		$merge = new Config($this->getDataFolder() . "merge.yml", 2);
		if($merge->exists($player->getLevel()->getName())) {
			$mergedata = [
				"$plot" => "$plot2",
				"$plot2" => "$plot"
			];
			$merge->setNested($player->getLevel()->getName() . "." . $plot, "$plot2");
			$merge->setNested($player->getLevel()->getName() . "." . $plot2, "$plot");
		} else {
		$mergedata = [
			"$plot" => "$plot2",
			"$plot2" => "$plot"
		];
		$merge->set($player->getLevel()->getName(), $mergedata);
	}
			//	$merge->set("$plot", "$plot2");
			//	$merge->set("$plot2", "$plot");
				$merge->save();
				$this->getScheduler()->scheduleTask(new RoadFillTask($this, $plot, $player, $direction, $maxBlocksPerTick));
		return true;
	}

	public function unmergePlot(Plot $plot , Plot $plot2 , Player $player, $direction, int $maxBlocksPerTick = 256) : bool {
		$this->newRandPlot($plot, 256, 44, 1);
		$this->newRandPlot($plot2, 256, 44, 1);
		$unmerge = new Config($this->getDataFolder() . "merge.yml", 2);
		//$unmerge->remove("$plot");
		//$unmerge->remove("$plot2");
		$unmerge->removeNested($player->getLevel()->getName() . "." . "$plot2");
		$unmerge->removeNested($player->getLevel()->getName() . "." . "$plot");
		$unmerge->save();
		$this->getScheduler()->scheduleTask(new UnMergeTask($this, $plot, $player, $direction, $maxBlocksPerTick));

		return true;
	}

    public function editMergeRand(Player $player) {
        $username = $player->getName();
        $merge = new Config($this->getDataFolder() . "merge.yml", 2);
        ...
        $nordplot = $this->getPlotByPosition($nordplot);
        $suedplot = $this->getPlotByPosition($suedplot);
        $eastplot = $this->getPlotByPosition($eastplot);
        $westplot = $this->getPlotByPosition($westplot);
        if($merge->getNested($player->getLevel()->getName() . ".$nordplot") or $merge->getNested($player->getLevel()->getName() . ".$suedplot") or $merge->getNested($player->getLevel()->getName() . ".$eastplot") or $merge->getNested($player->getLevel()->getName() . ".$westplot")) {
